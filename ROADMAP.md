...
### 🟨 Further scent-article expansion — active next-beta batch
...
Implemented in the active development batch; pending Windows/in-game verification:
...
### 🟨 Further track-direction realism — active next-beta batch
...
- Persistent saved containment positions with nearby-position restoration
...
### 🟨 10. K9 injury evacuation — active next-beta batch
...
Implemented through v0.23.1-beta:

- Handler carry with a flat animal-safe pose

Implemented in the active development batch; pending Windows/in-game verification:

- Emergency loading sequence
- Veterinary transport drive or escort
- Staged rehabilitation period with saved progress and return-to-duty clearance

Still to add:

- Assisted-walk animation
- Manual K9-down emergency command

### 🟨 14. Explosive-detection safety response — active next-beta batch
...
- A dedicated explosive perimeter marker separate from the exact evidence marker
...
- Successful patrol commands can award randomly sized, level-scaled XP with the approved 50% live-action markup.
- Successful eligible rewards can randomly add 1–3 confidence; hesitation/disobedience can randomly remove 1–3 confidence.
- The first two back-to-back uses retain full reward potential, later repetition diminishes, and continued nonproductive farming can remove XP/confidence.
- Genuine active Search and Track work is exempt from repetition penalties.

## Remaining unimplemented roadmap

### 🟨 12. Search evidence markers — active next-beta batch

Implemented for exact positive-alert coordinates, named vehicle/body zones, ten-minute temporary map markers, a Clear Evidence Markers command, and persistent CSV evidence records. In-game verification remains.

### 🟨 13. Vehicle-specific search behavior — active next-beta batch
...
The active implementation reads actual vehicle bones when available and falls back to category-aware motorcycle, normal-body, utility, and long-body routes with additional checkpoints. In-game verification across representative vehicle classes remains.

### 🟨 15. Water and obstacle tracking — active next-beta batch

The active implementation detects water, steep grade, traffic density, rail/fence/gate objects, construction barriers, and confined obstacle fields. Each can change scent quality, movement speed, and direction-check probability. It also performs a visible full-circle opening cast with training-, trust-, fatigue-, weather-, and scent-quality-based wrong-direction chances plus a strong leash-pull/body-line commitment. Pending Windows and in-game verification.

### 🟨 16. Multiple persistent K9 profiles — active next-beta batch
...
The active implementation adds an eight-dog persistent roster, separate profile/trust/medical/training files, active-dog selection, kennel assignment, duty status, and safe legacy Rex-profile migration. Pending Windows and in-game verification.
...
The active next-beta development priorities are:
...
5. Water and obstacle tracking, full-circle direction testing, variable wrong-direction behavior, strong route indication, and full scent-article quality expansion

The shared callout API and controlled Lost Child, Fugitive Trail from an Abandoned Vehicle, and Armed Burglary Suspect Hiding test set are deferred from this batch. Patrol-earned XP/confidence shipped in v0.23.1-beta. Continue pursuit cleanup, explosive-safety verification, exact marker cleanup controls, and multiple simultaneous containment positions alongside the active five priorities.
