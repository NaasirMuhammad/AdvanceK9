...
namespace AdvancedK9
{
    internal sealed class K9Controller : IDisposable
    {
        ...
        private uint _nextObstacleDirectionCheck;
        private string _activeScentSource="None";
        private ScentSample _activeScentSample;
        private Vector3 _lastSearchAlertPosition;
        private string _lastSearchAlertZone="target center";
        private readonly HashSet<PoolHandle> _explosiveSearchLockouts=new HashSet<PoolHandle>();
        ...
        private Vector3 _releasedBiteGroundPosition;
        private uint _releasedBiteGroundAt;
        private bool _releasedBiteFrozen;
        private uint _nextReleasedBiteGroundPose;
        ...
        private Blip _veterinaryBlip;
        private bool _mobileVetActive;
        private Vehicle _mobileVetVehicle;
        private Ped _mobileVet;
        private Blip _mobileVetBlip;
        private uint _nextRehabilitationSession;
        ...
        private void HandleApprehendShortcut()
        {
            ...
            if(ChordPressed(_config.ModifierKey,_config.ApprehendKey)||controllerPressed)
            {
                _immediateApprehendInput=true;
                try{Game.LogTrivial("AdvancedK9 immediate apprehend shortcut accepted.");Execute(K9Command.Apprehend);}
                finally{_immediateApprehendInput=false;}
            }
        }
        ...
        private void HandlePushToTalk()
        {
            ...
            {
                // Voice recognition can take longer than the normal aimed-target memory.
                // Preserve the last lawful target for the delayed transcription result.
                Ped aimed=GetValidAimedSuspect(false);
                if(aimed!=null){_voiceAimedTarget=aimed;_recentApprehendTarget=aimed;_recentApprehendTargetUntil=Game.GameTime+15000;}
            }
        ...
        private void DrainVoice(bool dogExists)
        {
            ...
            {
                if (dogExists || command == K9Command.SpawnDismiss)
                {
                    bool voiceApprehend=command==K9Command.Apprehend;
                    if(voiceApprehend)_immediateApprehendInput=true;
                    try{Execute(command);}
                    finally{if(voiceApprehend)_immediateApprehendInput=false;}
                }
            }
        }
        ...
        {
            CloseSeatCalibrationDoor();_menuMode="commands_root";_menu.Open("ADVANCED K9 — "+L("Commands").ToUpperInvariant(),new[]{L("Partner Control"),L("Search & Detection"),L("Tracking & Scent"),L("Tactical Deployment"),L("Vehicle & Equipment"),L("Care & Medical"),L("Training & Certifications"),"K9 Callouts",L("Deploy / Dismiss"),VoiceMenuLabel()});
        }
        ...
        private void OnMenuSelected(int index)
        {
            ...
            {
                if(index==0){string n=PromptForDogName(24);if(!string.IsNullOrWhiteSpace(n)){_profile.SetName(n);_roster.UpdateActive(_profile.Name,null,null);_voice?.UpdateWakeWord(_profile.Name);}}
        ...
        {
            _menuMode="roster";
            _menu.Open("K9 KENNEL ROSTER",_roster.Entries.Select(x=>(x.Id==_roster.ActiveId?"★ ":"")+x.Summary).Concat(new[]{"+ Add new K9 profile","Assign active K9 to nearest kennel","← "+L("Back to K9 Profile")}));
        }

        private void HandleRosterMenu(int index)
        {
            if(index<0)return;
            if(index<_roster.Entries.Count)
            {
                if(DogExists()){Game.DisplayNotification("~y~Return the deployed K9 before switching profiles.");return;}
                SwitchRosterProfile(_roster.Entries[index].Id);OpenRosterMenu();return;
            }
            if(index==_roster.Entries.Count)
            {
                if(DogExists()){Game.DisplayNotification("~y~Return the deployed K9 before adding a profile.");return;}
                string name=PromptForDogName(24);if(string.IsNullOrWhiteSpace(name))return;StationKennel kennel=NearestKennel(8f);
                var entry=_roster.Add(name,kennel==null?"":kennel.Key);if(entry==null){Game.DisplayNotification("~y~The kennel roster is full (8 K9s).");return;}
                SwitchRosterProfile(entry.Id);_profile.SetName(name);_roster.UpdateActive(name,entry.KennelKey,"Available");OpenRosterMenu();return;
            }
            if(index==_roster.Entries.Count+1)
            {
                StationKennel kennel=NearestKennel(8f);if(kennel==null){Game.DisplayNotification("~y~Stand beside a station kennel to assign this K9.");return;}
                _roster.UpdateActive(_profile.Name,kennel.Key,_profile.IsRehabilitating?"Rehabilitation":"Available");Game.DisplayNotification("~g~"+_profile.Name+" assigned to "+kennel.Name+".");OpenRosterMenu();return;
            }
            _menuMode="profile";RefreshProfileMenu();
        }

        private void SwitchRosterProfile(string id)
        {
            if(!_roster.Select(id))return;_profile.Save();_trust.Save();_profile=new K9Profile(_config,id);_trust=new TrustProfile(_config.StartingTrust,_config.ShowActionNotifications,id);_seatProfiles=new VehicleSeatProfiles(_config.VehicleSeatOffsetX,_config.VehicleSeatOffsetY,_config.VehicleSeatOffsetZ,id);
            _voice?.UpdateWakeWord(_profile.Name);Game.DisplayNotification("~b~Active K9 profile:~s~ "+_profile.Name+"~n~Kennel: "+(string.IsNullOrWhiteSpace(_roster.Active.KennelKey)?"unassigned":_roster.Active.KennelKey));
        }
        ...
        {
            if(Game.GameTime<_nextCarryPresentation)return;_nextCarryPresentation=Game.GameTime+1200;
            var handler=Game.LocalPlayer.Character;
            if(!NativeFunction.Natives.IS_ENTITY_PLAYING_ANIM<bool>(handler,"anim@heists@box_carry@","idle",3))
            ...
            if(!NativeFunction.Natives.IS_ENTITY_PLAYING_ANIM<bool>(_dog,"creatures@rottweiler@amb@sleep_in_kennel@","sleep_in_kennel",3))
        ...
        private void Execute(K9Command command)
        {
            try
            {
                ...
                switch (command)
                {
                    ...
                    case K9Command.EmergencyLoadK9: EmergencyLoadK9(); break;
                    case K9Command.VeterinaryTransport: BeginVeterinaryTransport(); break;
                    case K9Command.Rehabilitation: RehabilitationSession(); break;
        ...
        private void ReturnToKennel(StationKennel kennel)
        {
        ...
        private void Deploy(StationKennel kennel)
        {
            ...
            if(sleeping!=null&&sleeping.Exists()&&kennel.ResidentProfileId==_roster.ActiveId)
            {
                ...
                _state=K9State.Following;_deployed=true;_downed=false;
                _lastDogPosition=_dog.Position;_lastDogHeading=_dog.Heading;
            }
            else
            {
                Game.DisplayNotification("~y~The selected K9 is not visible in this kennel yet. Move closer and retry.");
                return;
            }
            _profile.RecordDeployment();
            _roster.RecordDuty(true);
            Follow();
        ...
        private void CompleteCollisionSafeVehicleEgress(Vehicle vehicle,Vector3 exit)
        {
            ...
            {
                NativeFunction.Natives.SET_ENTITY_COLLISION(_dog,true,true);
                Game.LogTrivial("AdvancedK9 vehicle exit: collision-safe egress completed from "+FormatVector(exit)+" before handler follow was assigned.");
                Follow();
            }
        }
        ...
        private void Bathroom()
        {
            ...
            {
                PlayCanineClip(_dog,"creatures@rottweiler@move","dump_loop",3800,true);
                if(DogExists())reliefPoint=_dog.GetOffsetPosition(new Vector3(0f,-.35f,0f));
                SpawnDogWaste(reliefPoint);_bowel=100;
                K9IncidentLog.Write(_profile.Name,"Care","Defecated automatically",reliefPoint);
            }
            if(DogExists())Follow();
        }
        ...
        {
            if(_mobileVetActive){Game.DisplayNotification("~y~The mobile K9 veterinarian is already responding.");return;}
        ...
        {
            Model vehicleModel=new Model("speedo"),vetModel=new Model("s_m_m_paramedic_01");
            try
            {
            ...
            while (_running && generation==_trackGeneration && DogExists() && target.Exists() && !target.IsDead && Game.GameTime < end && _state == K9State.Tracking)
            {
                ...
                if (_dog.DistanceTo(ladderTerminal?trackTerminal:target.Position) <= 4.5f)
                {
                    ...
                    if(calloutTrack)
                    {
                        ...
                        Game.LogTrivial("AdvancedK9 callout containment: Rex stopped at "+(ladderTerminal?"rooftop access "+FormatVector(trackTerminal):_dog.DistanceTo(target).ToString("0.0")+"m from target")+" and issued the immediate alert bark.");
                    }
                    else
                    {
                        Bark(2);
                ...
                var destination=routeIndex<route.Count?route[routeIndex]:trackTerminal;
                ...
                if(calloutTrack&&routeIndex==route.Count-1&&distance>4.5f)
                {
                    ...
                    waypoint=new Vector3(trackTerminal.X-dx/distance*approachDistance,trackTerminal.Y-dy/distance*approachDistance,trackTerminal.Z);
                }
                _dog.Tasks.Clear();
                ...
                if(calloutTrack&&routeIndex<route.Count&&(moved<0.8f||oscillationCount>=3)&&remaining>6f)
                {
                    ...
                    if(stalledAttempts>=2)
                    {
                        ...
                        Vector3 rooftopAccess;
                        if(!ladderTerminal&&TryFindLoadedRooftopAccess(target,out rooftopAccess))
                        {
                            trackTerminal=rooftopAccess;ladderTerminal=true;
        ...
        {
            return BuildCalloutNavigationRoute(start,destination,60f,0);
        }

        private List<Vector3> BuildCalloutNavigationRoute(Vector3 start,Vector3 destination,float maximumSpacing,int variant)
        {
            ...
            for(int i=1;i<segments;i++)
            {
                ...
                float curve=(float)Math.Sin(Math.PI*t)*lateral;
                Vector3 sample=new Vector3(start.X+lineX*t+perpendicularX*curve,start.Y+lineY*t+perpendicularY*curve,start.Z+(destination.Z-start.Z)*t);
                Vector3 waypoint;
        ...
        private bool TryResolvePedNavigationPoint(Vector3 requested,float expectedZ,out Vector3 resolved)
        {
            resolved=requested;
            try
            {
                ...
                float ground;
                if(NativeFunction.Natives.GET_GROUND_Z_FOR_3D_COORD<bool>(nav.X,nav.Y,nav.Z+5f,out ground,false)&&Math.Abs(ground-expectedZ)<=3.25f)nav=new Vector3(nav.X,nav.Y,ground);
                resolved=nav;
                return true;
            }
            catch{return false;}
        ...
        private bool TryFindLoadedRooftopAccess(Ped target,out Vector3 access)
        {
            access=Vector3.Zero;
            if(!DogExists()||target==null||!target.Exists())return false;
            float dx=target.Position.X-_dog.Position.X,dy=target.Position.Y-_dog.Position.Y;
            if((float)Math.Sqrt(dx*dx+dy*dy)>75f||target.Position.Z-_dog.Position.Z<4.5f)return false;
            try
            {
                var ladder=World.GetAllObjects()
                    .Where(o=>o!=null&&o.Exists()&&o.DistanceTo(target)<55f&&o.Position.Z<target.Position.Z-1.5f&&
                        (o.Model.Name??"").IndexOf("ladder",StringComparison.OrdinalIgnoreCase)>=0)
                    .OrderBy(o=>o.DistanceTo(target)).FirstOrDefault();
                if(ladder==null||!ladder.Exists())return false;
                Vector3 nav;
                if(!TryResolvePedNavigationPoint(ladder.Position,ladder.Position.Z,out nav))nav=ladder.Position;
                access=nav;return true;
            }
            catch{return false;}
        }
        ...
        {
            var result=new TrailEnvironment();float distance=Math.Max(.1f,from.DistanceTo(destination));float grade=Math.Abs(destination.Z-from.Z)/distance;
            if(grade>.24f){result.Label="Steep terrain direction check";result.QualityPenalty=2;result.SpeedMultiplier=.68f;result.DirectionCheckChance=35;return result;}
            try
            {
            ...
            try
            {
                var nearby=World.GetAllObjects().Where(o=>o.Exists()&&o.DistanceTo(midpoint)<8f).Take(24).ToList();string names=string.Join(" ",nearby.Select(o=>(o.Model.Name??"").ToLowerInvariant()));
                if(names.Contains("rail")||names.Contains("track")){result.Label="Railroad crossing";result.QualityPenalty=3;result.SpeedMultiplier=.72f;result.DirectionCheckChance=45;}
                else if(names.Contains("fence")||names.Contains("gate")){result.Label="Fence / gate obstacle";result.QualityPenalty=3;result.SpeedMultiplier=.62f;result.DirectionCheckChance=50;}
        ...
        private void BeginReleasedBiteMedicalHold(bool notify)
        {
            if(_controlledBiteTarget!=null&&_controlledBiteTarget.Exists()&&!_controlledBiteTarget.IsDead)
            {
                ...
                _releasedBiteGroundPosition=_releasedBiteTarget.Position;
                _releasedBiteGroundAt=Game.GameTime+500;
                _releasedBiteFrozen=false;
                _nextReleasedBiteGroundPose=0;
        ...
        private void PlayControlledBiteVisual(Ped target,bool firstContact)
        {
            ...
            try
            {
                ...
                if(NativeFunction.Natives.HAS_ANIM_DICT_LOADED<bool>(dictionary))
                {
                    ...
                    Vector3 local=NativeFunction.Natives.GET_OFFSET_FROM_ENTITY_GIVEN_WORLD_COORDS<Vector3>(
                        target,_dog.Position.X,_dog.Position.Y,_dog.Position.Z);
                    string clip=local.Y<0f?"dog_takedown_from_back":"dog_takedown_from_front";
                    NativeFunction.Natives.TASK_PLAY_ANIM(_dog,dictionary,clip,8f,-4f,1550,0,0f,false,false,false);
        ...
        private void MaintainReleasedBiteTarget()
        {
            try
            {
                ...
                if(_releasedBiteTreated)
                {
                    if(_releasedBiteFrozen){NativeFunction.Natives.FREEZE_ENTITY_POSITION(_releasedBiteTarget,false);_releasedBiteTarget.Tasks.Clear();_releasedBiteFrozen=false;}
                    NativeFunction.Natives.TASK_HANDS_UP(_releasedBiteTarget,-1,Game.LocalPlayer.Character,-1,true);
                }
                else if(Game.GameTime>=_releasedBiteGroundAt&&Game.GameTime>=_nextReleasedBiteGroundPose)
                {
                    _nextReleasedBiteGroundPose=Game.GameTime+3500;
        ...
        {
            if(_releasedBiteTarget==null||!_releasedBiteTarget.Exists()||_releasedBiteTarget.IsDead)return;
            const string dictionary="combat@damage@writhe";const string animation="writhe_loop";
            try
            {
                bool playing=NativeFunction.Natives.IS_ENTITY_PLAYING_ANIM<bool>(_releasedBiteTarget,dictionary,animation,3);
                ...
                if(NativeFunction.Natives.HAS_ANIM_DICT_LOADED<bool>(dictionary))
                {
                    ...
                    _releasedBiteTarget.Position=_releasedBiteGroundPosition;
                    NativeFunction.Natives.TASK_PLAY_ANIM(_releasedBiteTarget,dictionary,animation,4f,-2f,-1,1,0f,false,false,false);
                    _releasedBiteFrozen=true;
                    Game.LogTrivial("AdvancedK9 bite patient placed into a persistent downed medical animation before PR could restore standing idle.");
                }
                else
                {
                    NativeFunction.Natives.SET_PED_TO_RAGDOLL(_releasedBiteTarget,8000,10000,0,false,false,false);
        ...
        {
            if(_releasedBiteTarget!=null&&_releasedBiteTarget.Exists()){if(_releasedBiteFrozen)NativeFunction.Natives.FREEZE_ENTITY_POSITION(_releasedBiteTarget,false);_pr.CompleteK9Custody(_releasedBiteTarget);}
            ...
            _releasedBiteGroundPosition=Vector3.Zero;_releasedBiteGroundAt=0;_releasedBiteFrozen=false;_nextReleasedBiteGroundPose=0;
        }
        ...
        private void MaintainSwimming()
        {
            ...
            else if(!inWater&&_swimming)
            {
                _swimming=false;
                NativeFunction.Natives.STOP_ANIM_TASK(_dog,"creatures@rottweiler@swim@","swim",1f);
                if(_state==K9State.Following||_state==K9State.Leashed)IssuePersistentFollow(Game.LocalPlayer.Character,_state==K9State.Leashed);
            }
        }
        ...
        private void MaintainFollowNavigation()
        {
            ...
            {
                Vector3 offset=leashed?new Vector3(-.85f,.6f,.1f):new Vector3(-1.05f,1.0f,.1f);
        ...
        public void Dispose()
        {
            ...
            _voice?.Dispose();
            _voice = null;
            if(_veterinaryBlip!=null&&_veterinaryBlip.Exists())_veterinaryBlip.Delete();
            _hud.Dispose();
            _evidenceMarkers.Dispose();
            _trust.Save();
            _profile.Save();
            Dismiss(false);
            DeleteStationKennels();
