...
namespace AdvancedK9.Callouts
{
    ...
    public sealed class ArmedBurglaryCallout : AdvancedK9Callout
    {
        ...
        private bool _pacificSwatDeploymentStarted;
        private readonly List<Ped> _pacificSwatTeam=new List<Ped>();
        private Blip _primaryContactBlip;
        ...
        private void GetBankPerimeterLayout(out List<Vector3> positions,out List<float> headings)
        {
            ...
            if(_sceneIndex==0)
            {
                // Tester-recorded Pacific Standard eight-cruiser traffic closure.
                positions.Add(new Vector3(287.32f,174.82f,104.30f));headings.Add(156.5f);
                positions.Add(new Vector3(286.45f,170.43f,104.37f));headings.Add(161.1f);
                positions.Add(new Vector3(201.07f,168.42f,105.50f));headings.Add(69.1f);
                positions.Add(new Vector3(195.97f,170.19f,105.59f));headings.Add(64.0f);
                positions.Add(new Vector3(138.80f,208.38f,106.95f));headings.Add(334.8f);
                positions.Add(new Vector3(140.65f,212.14f,107.01f));headings.Add(355.8f);
                positions.Add(new Vector3(222.80f,263.96f,105.60f));headings.Add(252.6f);
                positions.Add(new Vector3(225.25f,263.24f,105.60f));headings.Add(260.7f);
                return;
            }
            if(_sceneIndex==1)
            {
            ...
            if(_sceneIndex==2)
            {
                ...
                // approaches while keeping the bank entrance and command post clear.
                positions.Add(new Vector3(352.45f,-243.98f,53.84f));headings.Add(252.8f);
                positions.Add(new Vector3(377.39f,-270.60f,53.93f));headings.Add(140.2f);
                positions.Add(new Vector3(380.03f,-266.66f,53.76f));headings.Add(146.4f);
                positions.Add(new Vector3(347.78f,-289.62f,53.96f));headings.Add(64.9f);
                positions.Add(new Vector3(252.06f,-240.80f,53.97f));headings.Add(341.3f);
                positions.Add(new Vector3(254.59f,-235.84f,54.06f));headings.Add(357.4f);
                positions.Add(new Vector3(315.75f,-246.45f,53.94f));headings.Add(240.6f);
                return;
            }
            if(_sceneIndex==3)
            {
        ...
        private bool StageBankPerimeter()
        {
            ...
            if(_sceneIndex==0)
            {
                Vector3 pacificSwatStaging=new Vector3(148.48f,185.56f,106.03f);float pacificSwatHeading=337.9f;
                _swatBearcat=SpawnVehicle("riot",pacificSwatStaging,pacificSwatHeading,true);
                if(_swatBearcat==null||!_swatBearcat.Exists())_swatBearcat=SpawnVehicle("fbi2",pacificSwatStaging,pacificSwatHeading,true);
                ...
                Vector3 airOrbitCenter=new Vector3(205.20f,201.90f,173.50f);
                _airOne=SpawnVehicle("polmav",airOrbitCenter,heading,true);
                ...
                NativeFunction.Natives.TASK_HELI_MISSION(pilot,_airOne,0,0,205.20f,201.90f,105.50f,9,24f,65f,-1f,90,45,18f,0);
            }
        ...
        private bool AcceptAndRouteToSelectedBank(string instruction)
        {
            ...
            {
                _primaryContactBlip=new Blip(OfficerOne);_primaryContactBlip.Name="PRIMARY CONTACT — BANK COMMAND";_primaryContactBlip.Color=Color.DodgerBlue;
            }
            Game.DisplayHelp("GPS route set to ~y~"+BankNames[_sceneIndex]+"~s~. The ~b~blue officer marker~s~ identifies primary bank command.");
        ...
        private void StageBankManager()
        {
            ...
            Vehicle commandVehicle=PoliceVehicle;
            if(_sceneIndex==2)
            {
                // Hawick's first mapped cruiser is an outer lane blocker on the
                // exposed Elgin approach. Use mapped cruiser four as the protected
                // witness/command position instead of placing the manager roadside.
                List<Vehicle> hawickUnits=AllBankPoliceVehicles();
                if(hawickUnits.Count>3&&hawickUnits[3]!=null&&hawickUnits[3].Exists())commandVehicle=hawickUnits[3];
            }
            Vector3 managerPosition=commandVehicle!=null&&commandVehicle.Exists()?BankCoverPosition(commandVehicle,0f):new Vector3(Scene.X+12f,Scene.Y+12f,Scene.Z);
            ...
            Game.LogTrivial("AdvancedK9 Callouts: bank manager secured behind the protected command vehicle and assigned as the command-post witness"+(_sceneIndex==2?"; Hawick exposed Elgin staging excluded.":"."));
        }
        ...
        {
            // Preserve the shirt where it was torn off inside the bank. The
            // manager remains at scene command and is not used as an evidence
            // staging point.
            Vector3 position=InteriorRobberPositions[_sceneIndex];
            ...
            if(EvidenceProp!=null&&EvidenceProp.Exists())
            {
                ...
                Game.LogTrivial("AdvancedK9 Callouts: torn clothing preserved inside the bank at the beginning of the suspect's escape trail at "+EvidenceProp.Position+".");
            }
        }
        ...
        private void MaintainBankTacticalPosture()
        {
            ...
            for(int i=0;i<officers.Count;i++)
            {
                ...
                if(_pacificSwatDeploymentStarted&&_pacificSwatTeam.Contains(officer))continue;
        ...
        private void ActivateBankEntryTeam()
        {
            ...
            // Pacific uses its dedicated four-person SWAT team for every interior
            // entry.  The sixteen patrol officers remain on their assigned exterior
            // traffic/perimeter posts.  Standard banks use four patrol officers.
            List<Ped> officers=_sceneIndex==0&&_pacificSwatTeam.Count==4?new List<Ped>(_pacificSwatTeam):AllBankOfficers();
            for(int i=0;i<officers.Count&&_bankEntryTeam.Count<4;i++)_bankEntryTeam.Add(officers[i]);
            Game.DisplayNotification(_sceneIndex==0?"~b~Pacific SWAT:~s~ Four operators will enter with you. Patrol is maintaining the exterior perimeter.":"~b~Bank arrest team:~s~ Four on-scene officers will move with you to secure the surrendering suspects.");
            Game.LogTrivial(_sceneIndex==0?"AdvancedK9 Callouts: Pacific four-person SWAT team designated as the player-led entry element; all patrol officers retain exterior perimeter assignments.":"AdvancedK9 Callouts: four existing bank-scene officers designated as the player-led entry/arrest element.");
        ...
        private void UpdateBankEntryTeam(Ped player,bool force)
        {
            ...
            {
                int playerInterior=NativeFunction.Natives.GET_INTERIOR_FROM_ENTITY<int>(player);
                int subjectInterior=Subject!=null&&Subject.Exists()?NativeFunction.Natives.GET_INTERIOR_FROM_ENTITY<int>(Subject):0;
                bool playerCommitted=playerInterior!=0&&(subjectInterior==0||playerInterior==subjectInterior);
                if((playerCommitted||player.DistanceTo(Scene)<18f)&&!_bankEntryMoving)
                {
                    _bankEntryMoving=true;
                    for(int i=0;i<_bankEntryTeam.Count;i++)
                    {
                        Ped officer=_bankEntryTeam[i];if(officer==null||!officer.Exists())continue;
                        float side=i%2==0?-1.8f:1.8f;float rear=-2.5f-(i/2)*1.5f;
                        NativeFunction.Natives.TASK_FOLLOW_TO_OFFSET_OF_ENTITY(officer,player,side,rear,0f,3.4f,-1,1.4f,true);
                        NativeFunction.Natives.SET_PED_KEEP_TASK(officer,true);
                    }
                    Game.LogTrivial("AdvancedK9 Callouts: player crossed the bank entry threshold; four-officer arrest element is following through the interior.");
                }
                if(force||Game.GameTime>=_nextBankTacticalRefresh)
                {
                    ...
                    for(int i=0;i<_bankEntryTeam.Count;i++)
                    {
                        ...
                        uint weapon=NativeFunction.Natives.GET_HASH_KEY<uint>(i==0?"WEAPON_STUNGUN":"WEAPON_COMBATPISTOL");
                        NativeFunction.Natives.SET_CURRENT_PED_WEAPON(officer,weapon,true);
                        if(!_bankEntryMoving)NativeFunction.Natives.TASK_AIM_GUN_AT_ENTITY(officer,i<2?Subject:_bankAccomplices.Count>0?_bankAccomplices[0]:Subject,-1,false);
                        NativeFunction.Natives.SET_PED_KEEP_TASK(officer,true);
                    }
                }
        ...
        public override bool OnCalloutAccepted()
        {
            ...
            if(!AcceptAndRouteToSelectedBank("Officers preserved clothing torn from the fleeing suspect inside the bank."))return false;
            DispatchUpdate("CalloutAccepted","Bank robbery foot escape","Local patrol jurisdiction","No confirmed getaway vehicle","Armed bank-robbery suspect","Fled on foot from the bank","Possibly armed","Respond to a reported bank robbery. The manager is secured at scene command, and officers preserved a clothing scent article inside the bank where it was torn from the fleeing suspect.","WE_HAVE CRIME_BURGLARY IN_OR_ON_POSITION UNITS_RESPOND_CODE_3",Scene);
            return true;
        }
        ...
        private bool InitializeBankContainmentScenario(bool pacificInterior)
        {
            ...
            NativeFunction.Natives.SET_PED_KEEP_TASK(_hostage,true);
        ...
        private void UpdateEstablishSceneBriefing(Ped player)
        {
            ...
            Game.DisplayHelp("The ~b~PRIMARY CONTACT — BANK COMMAND~s~ officer is your scene contact. Bank-aware spoken dialogue will activate after the NPC AI Callout API is installed and integrated.");
        ...
        public override void Process()
        {
            ...
            if(!_sceneBriefed&&player.DistanceTo(Scene)<35f)
            {
                ...
                AssignCalloutScent(Subject,article,"torn shirt preserved inside the bank","A visible piece of the suspect's torn shirt is preserved inside the bank near the start of the escape trail. Bring Rex to it and command COLLECT SCENT or TRACK.");
        ...
        private void ProcessBankContainment(Ped player)
        {
            ...
            if(_bankScenarioStarted&&!ApiRequested&&K9Available())
            {
                AssignCalloutScent(Subject,Scene,"LiveHumanOdor: fresh robber odor venting from the occupied bank and bank-entry threshold","Rex can isolate the robbers' fresh human trail without clothing. At the bank entrance, command SEARCH AREA or TRACK.");
                if(ApiRequested)Game.LogTrivial("AdvancedK9 Callouts: occupied-bank live human-odor source registered at the entry threshold; no clothing article required.");
            }
        ...
        private void ProcessPlayerNegotiation(Ped player)
        {
            ...
            if(_negotiationRound==0)Game.DisplayHelp("~b~Initial contact:~s~ [1] Demand proof of life  [2] Ask who is in charge  [3] Threaten immediate entry");
            else if(_negotiationRound==1)Game.DisplayHelp("~b~Establish demands:~s~ [1] Ask what they need  [2] Ask about weapons/injuries  [3] Order an immediate surrender");
            else if(_negotiationRound==2)Game.DisplayHelp("~b~Build cooperation:~s~ [1] Offer fair treatment  [2] Offer food/medical aid  [3] Reject further discussion");
            else if(_negotiationRound==3)Game.DisplayHelp("~b~Negotiate release:~s~ [1] Demand the hostage first  [2] Accept vehicle and clear passage  [3] Refuse all demands");
            else Game.DisplayHelp("~b~Final resolution:~s~ [1] Order a peaceful surrender  [2] Confirm hostage-for-vehicle terms  [3] End the call and authorize entry");
            if(!anyChoice){_negotiationChoiceHeld=false;return;}
            if(_negotiationChoiceHeld)return;
            _negotiationChoiceHeld=true;_nextNegotiationAt=Game.GameTime+1800;
            if(_negotiationRound==0)
            {
                ...
                else if(choice2){_negotiationScore+=1;Game.DisplayNotification("~b~You:~s~ Who am I speaking with, and who is inside?~n~~r~Robber:~s~ I am in charge. Two of us and one employee. Keep everyone back.");}
            ...
            {
                if(choice1){_negotiationScore+=1;Game.DisplayNotification("~b~You:~s~ Tell me exactly what you need to end this safely.~n~~r~Robber:~s~ An unmarked car, a clear route, and no one following us.");}
                else if(choice2){_negotiationScore+=2;Game.DisplayNotification("~b~You:~s~ Is anyone hurt, and how many weapons are inside?~n~~r~Robber:~s~ Nobody is hurt. We are armed, but we do not want a shootout.");}
                else {_negotiationScore-=2;Game.DisplayNotification("~b~You:~s~ Put the weapons down and surrender now.~n~~r~Robber:~s~ Not until we know we are walking out alive.");}
            }
            else if(_negotiationRound==2)
            {
                if(choice1){_negotiationScore+=2;Game.DisplayNotification("~b~You:~s~ Cooperate and I will ensure everyone is treated fairly.~n~~r~Robber:~s~ Then prove it. Keep the rifles outside and bring us a car.");}
                else if(choice2){_negotiationScore+=2;Game.DisplayNotification("~b~You:~s~ We can send food or medical help if you release one hostage.~n~~r~Robber:~s~ No one comes in, but I may send the employee out.");}
                else {_negotiationScore-=3;Game.DisplayNotification("~b~You:~s~ We are done talking.~n~~r~Robber:~s~ Then this gets worse.");}
            }
            else if(_negotiationRound==3)
            {
                if(choice1){_negotiationScore+=2;Game.DisplayNotification("~b~You:~s~ The hostage comes out before anything moves.~n~~r~Robber:~s~ Clear the front and the employee walks out alone.");}
                else if(choice2){_negotiationScore+=1;_escapeTermsAccepted=true;Game.DisplayNotification("~b~You:~s~ Release the hostage and an unmarked car will be staged outside.~n~~r~Robber:~s~ Agreed, if the route stays clear.");}
                else {_negotiationScore-=2;Game.DisplayNotification("~b~You:~s~ No car and no clear route. Release the hostage.~n~~r~Robber:~s~ Then give us a reason to trust you.");}
            }
            else
            {
                if(choice1){_negotiationScore+=3;_escapeTermsAccepted=false;Game.DisplayNotification("~b~You:~s~ Send the hostage out, place the weapons down, and walk out with empty hands.~n~~r~Robber:~s~ Give us a moment. We are coming out.");}
                else if(choice2){_negotiationScore+=1;_escapeTermsAccepted=true;Game.DisplayNotification("~b~You:~s~ The hostage exits first. Then you move to the staged vehicle.~n~~r~Robber:~s~ Understood. The employee is coming out now.");}
                else {_negotiationScore-=5;_negotiationForcedEntry=true;Game.DisplayNotification("~b~You:~s~ Negotiations are over. Entry team, stand by.~n~~r~Robber:~s~ Then come and get us.");}
            }
            _negotiationRound++;
            if(_negotiationRound>=5)ResolveBankNegotiation(player);
        }
        ...
        private void BeginHostileBankResponse(Ped player)
        {
            ...
            if(_sceneIndex==0)DeployPacificSwatForBreach();
        ...
        private void DeployPacificSwatForBreach()
        {
            if(_pacificSwatDeploymentStarted||_swatBearcat==null||!_swatBearcat.Exists()||_pacificSwatTeam.Count<4)return;
            _pacificSwatDeploymentStarted=true;
            Vehicle bearcat=_swatBearcat;var team=new List<Ped>(_pacificSwatTeam);
            GameFiber.StartNew(delegate
            {
                try
                {
                    ...
                    {
                        Ped swat=team[i];if(swat==null||!swat.Exists())continue;
                        swat.Tasks.ClearImmediately();NativeFunction.Natives.SET_PED_INTO_VEHICLE(swat,bearcat,i==0?-1:i-1);
                    }
                    Vector3 front=World.GetNextPositionOnStreet(new Vector3(238.0f,207.0f,105.5f));
                    Ped driver=team[0];
                    NativeFunction.Natives.SET_VEHICLE_SIREN(bearcat,true);NativeFunction.Natives.SET_VEHICLE_LIGHTS(bearcat,2);
                    NativeFunction.Natives.TASK_VEHICLE_DRIVE_TO_COORD_LONGRANGE(driver,bearcat,front.X,front.Y,front.Z,18f,786603,5f);
                    uint deadline=Game.GameTime+30000;
                    while(!Finished&&bearcat.Exists()&&driver.Exists()&&bearcat.DistanceTo(front)>9f&&Game.GameTime<deadline)GameFiber.Wait(250);
                    for(int i=0;i<team.Count;i++)
                    {
                        Ped swat=team[i];if(swat==null||!swat.Exists())continue;
                        NativeFunction.Natives.TASK_LEAVE_VEHICLE(swat,bearcat,0);
                    }
                    GameFiber.Wait(1600);
                    for(int i=0;i<team.Count;i++)
                    {
                        Ped swat=team[i];if(swat==null||!swat.Exists())continue;
                        Vector3 stack=Scene+HeadingVector(90f)*(i*1.1f-1.65f);
                        NativeFunction.Natives.TASK_FOLLOW_NAV_MESH_TO_COORD(swat,stack.X,stack.Y,stack.Z,2.4f,15000,1.2f,0,0f);
                        NativeFunction.Natives.SET_PED_KEEP_TASK(swat,true);
                    }
                    Game.DisplayNotification("~r~Pacific breach authorized:~s~ The BearCat and four-officer SWAT team have rolled from staging to the bank entrance.");
                    Game.LogTrivial("AdvancedK9 Callouts: Pacific SWAT deployed from tester coordinate 9 to the front approach only after negotiations failed.");
                }
                catch(System.Exception ex){Game.LogTrivial("AdvancedK9 Callouts: Pacific SWAT breach deployment contained: "+ex.Message);}
            },"AdvancedK9 Pacific SWAT breach deployment");
        }
        ...
        public override void End()
        {
            ...
            if(_primaryContactBlip!=null&&_primaryContactBlip.Exists())_primaryContactBlip.Delete();
